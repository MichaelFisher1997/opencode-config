...
**Important:** Always use `bunx convex dev` during development. Only use `bunx convex deploy` when you're ready to ship to production.
...
cd my-app
bun install convex @workos-inc/authkit-react
bunx convex dev
```
...
cd my-app
bun install convex @workos-inc/authkit-nextjs
bunx convex dev
```
...
- [ ] `bun install convex` completed
- [ ] `bunx convex dev` running (use this, NOT deploy!)
...
- [ ] When ready for production, use `bunx convex deploy`

**Remember:** Use `bunx convex dev` for all development work. Only use `bunx convex deploy` when deploying to production!
...
### Types not updating
- Run `bunx convex dev` (regenerates types)
